document.addEventListener('DOMContentLoaded', () => {
    ...
    function displayWords(words) {
        ...
        words.sort((a,b) => a.english.localeCompare(b.english)).forEach(word => {
            ...
            // ★★★★★ [철칙 준수] 영어는 원본 그대로 표시합니다. ★★★★★
            const engHtml = `<span class="word-item-eng">${word.english}</span>`;
            
            // ★★★★★ [요구사항 3] 품사 뒤에 공백을 추가하여 가독성을 높입니다. ★★★★★
            const posHtml = posText ? `<span style="font-size: 0.9em; color: var(--primary-color); margin-left: 4px;">${posText}</span> ` : '';
            const korHtml = `<span class="word-item-kor">${summary}</span>`;

            item.innerHTML = engHtml + posHtml + korHtml;
    ...
    // ★★★★★ [요구사항 2] TTS는 영어 파트 전체를 읽습니다. ★★★★★
    speakButton.addEventListener('click', () => {
        if (currentWord && 'speechSynthesis' in window) {
            // "English 필드는 신성불가침이다" 원칙을 준수합니다.
