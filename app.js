document.addEventListener('DOMContentLoaded', () => {
    ...
    // ★★★★★ [최종 논리] 통합 한국어 필드 해석기 (The Unified Korean Field Parser) ★★★★★
    const KNOWN_PARTS_OF_SPEECH = new Set(['n', 'v', 'vi', 'vt', 'a', 'ad', 'adj', 'pron', 'prep', 'conj', 'x', 'xconj']);
    const POS_PATTERN = /^[a-z]+$/; // 품사의 '생김새' (오직 소문자 영어)
    
    function parseKoreanField(koreanStr) {
        if (!koreanStr) return { pos: [], meanings: '' };

        const tokens = koreanStr.split(/[, ]+/).filter(t => t); // 쉼표와 공백으로 단어(토큰) 분해
        let boundaryIndex = 0; // 품사와 뜻의 경계 인덱스

        for (const token of tokens) {
            // 조건 A (형태)와 조건 B (지식)를 교차 검증
        ...
        // 원본 문자열에서 뜻 부분을 정확히 추출 (가장 안전한 방법)
        let meanings = '';
        ...
        // 안전장치: 만약 위 로직이 실패하여 뜻이 비어있다면, 원본 전체를 뜻으로 간주
        if (meanings.length === 0 && pos.length > 0) {
            return { pos: [], meanings: koreanStr };
        }
    ...
    function importDataToDB(wordsData) {
        return new Promise((resolve, reject) => {
            ...
            wordsData.forEach(word => {
                // ★★★★★ [최종 논리 적용] 오직 '뜻' 부분만 검색 태그로 만듭니다. ★★★★★
                const parsed = parseKoreanField(word.korean);
                word.searchTags = parsed.meanings.split(/[,/]/).map(tag => tag.trim()).filter(tag => tag.length > 0);
                
                // ★★★★★ [원칙 준수] english 필드는 절대 건드리지 않고, word 객체 그대로 저장합니다. ★★★★★
                store.add(word);
            });
    ...
    // (openDB, searchWords 등 다른 DB 함수는 변경 없음)
    function openDB() {
        return new Promise((resolve, reject) => {
            ...
            request.onupgradeneeded = e => {
                const db = e.target.result; let store;
    ...
    function searchWords(term) {
        // (지능형 검색으로 업그레이드)
        const searchTerms = term.trim().split(/\s+/).filter(t => t);
        if (searchTerms.length === 0) return Promise.resolve([]);
        
        const promises = searchTerms.map(singleTerm => singleTermSearch(singleTerm));
        
        return Promise.all(promises).then(resultsArrays => {
            const combinedResults = new Map();
            resultsArrays.forEach(arr => {
                arr.forEach(word => combinedResults.set(word.id, word));
            });
            return Array.from(combinedResults.values());
        });
    }
    function singleTermSearch(term) {
         return new Promise((resolve, reject) => {
            const tx = db.transaction(STORE_NAME, "readonly");
            const store = tx.objectStore(STORE_NAME);
            const results = new Map();
            const searches = [
                cursorSearch(store.index("english_idx"), IDBKeyRange.bound(term.toLowerCase(), term.toLowerCase() + '\uffff')),
                cursorSearch(store.index("korean_idx"), IDBKeyRange.bound(term, term + '\uffff')),
                cursorSearch(store.index("searchTags_idx"), IDBKeyRange.only(term))
            ];
    ...
    function displayWords(words) {
        ...
        words.sort((a,b) => a.english.localeCompare(b.english)).forEach(word => {
            ...
            // ★★★★★ [최종 논리 적용] 영어는 원본 그대로, 한글은 파싱 결과를 조합하여 표시 ★★★★★
            const parsed = parseKoreanField(word.korean);
            const posHtml = parsed.pos.length > 0 ? `<span style="font-size: 0.9em; color: var(--primary-color); margin-left: 4px;">(${parsed.pos.join(', ')})</span>` : '';
            ...
            korSpan.innerHTML = summary;
            
            item.appendChild(engSpan);
            item.innerHTML += posHtml; // 품사를 영어 뒤에 붙임
    ...
    function showDetailView(word) {
        ...
        // ★★★★★ [원칙 준수] 영어는 원본 그대로 표시합니다. ★★★★★
        flashcardEng.textContent = word.english;
        
        // ★★★★★ [최종 논리 적용] 한글은 파싱 결과를 조합하여 완벽하게 표시합니다. ★★★★★
        const parsed = parseKoreanField(word.korean);
    ...
    speakButton.addEventListener('click', () => {
        if (currentWord && 'speechSynthesis' in window) {
            const pureWord = currentWord.english.split(' ')[0]; // 복합 명사의 첫 단어만 읽음
